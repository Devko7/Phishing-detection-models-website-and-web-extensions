...
def load_llm_generated_dataset() -> pd.DataFrame:
    """
    Load and combine the LLM-generated phishing and legitimate email datasets.
    ...
    data_dir = project_root / "email-data" / "data" / "llm-generated"

    phishing_path = data_dir / "phishing.csv"
    legit_path = data_dir / "legit.csv"

    phishing_df = robust_read_csv(phishing_path)
    legit_df = robust_read_csv(legit_path)

    # CSV label column is unreliable (both files use 1); derive from source file.
    phishing_df["label"] = 1
    legit_df["label"] = 0
...
def train_and_evaluate(max_depth: int | None = None) -> Pipeline:
    """
    Train the decision tree classifier on the LLM-generated dataset
    and print a simple evaluation report.

    Returns the fitted Pipeline instance.
    """
    df = load_llm_generated_dataset()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Decision tree classifier for LLM-generated phishing vs. legitimate emails."
    )
    parser.add_argument(
        "--max-depth",
        type=int,
    ...
    save_path = Path(__file__).resolve().parent / "llm_email_phishing_classifier.joblib"
